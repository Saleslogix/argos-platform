...
The compiled web app is the overlay of two independent build outputs, not one. The SalesLogix release build (`products/argos-saleslogix/build/release.cmd`) writes `products/argos-saleslogix/deploy/`, which supplies `content/javascript/argos-saleslogix.js`, `content/dojo/dojo-dependencies.js`, `configuration/`, `localization/`, `help/`, and `content/css/app.min.css`. The SDK release build (`argos-sdk/build/release.cmd`) writes `argos-sdk/deploy/`, which supplies `content/javascript/argos-dependencies.js`, `content/javascript/argos-amd-dependencies.js`, `content/javascript/argos-sdk.js`, the Dojo core under `content/dojo/`, `content/javascript/cultures/`, and the CSS themes (including `content/css/theme-soho-light.min.css` and `content/css/themes/crm/sdk.min.crm.css`). The production IIS deploy assembles the running app by overlaying both folders into the same directory (the Jenkinsfile `iiscopy` step unstashes `slx` then `sdk`). Staging only the SalesLogix layer yields a blank page because none of the SDK / Dojo `<script>` tags in the entry template resolve; the Stager must therefore overlay `argos-sdk/deploy/` on top of `products/argos-saleslogix/deploy/`.

A single Cordova project lives in the `cordova/` directory at the monorepo root (sibling to `argos-sdk/`, `products/`, `packages/`, and `tests/`) and is driven by a new Grunt task (`grunt cordova`) that the Jenkinsfile invokes after the existing web build has produced both `products/argos-saleslogix/deploy/` and `argos-sdk/deploy/`. The `cordova/www/` directory is treated as a build artifact and is regenerated from both deploy folders on every build.

The Cordova shell ships its own version-controlled `index.html` template at `cordova/www-template/index.html`. Stager copies the `products/argos-saleslogix/deploy/` web content into `cordova/www/`, overlays `argos-sdk/deploy/` on top of it, and then overwrites `cordova/www/index.html` with this template. The template mirrors the bootstrap shape of the current `products/argos-saleslogix/index.html` (Soho config, Dojo AMD configuration, package map, `crm/polyfills/index` + `crm/Bootstrap` invocation, supported locales, configuration module, application module, locale files, regional files, root element) and adds the Cordova-specific pieces: a `<script src="cordova.js">` reference and a `deviceready` wrapper around the `crm/Bootstrap` call. The legacy `products/argos-saleslogix/index-phonegap.html` file is treated as a historical reference for the `deviceready` pattern only and is not copied into the Cordova bundle, because its localization configuration, `dojoConfig` shim, and `<body>` placement are out of date relative to the current `index.html`.

## Glossary

- **Argos_Web_App**: The compiled mobile web application. It is the overlay of two build outputs: the SalesLogix layer at `products/argos-saleslogix/deploy/` (produced by `products/argos-saleslogix/build/release.cmd`) and the SDK layer at `argos-sdk/deploy/` (produced by `argos-sdk/build/release.cmd`). Contents include `index.html`, `content/`, `configuration/`, `localization/`, and `help/`.
- **SLX_Deploy**: The SalesLogix layer at `products/argos-saleslogix/deploy/`. Supplies `content/javascript/argos-saleslogix.js`, `content/dojo/dojo-dependencies.js`, `configuration/`, `localization/`, `help/`, and `content/css/app.min.css`.
- **SDK_Deploy**: The SDK layer at `argos-sdk/deploy/`. Supplies `content/javascript/argos-dependencies.js`, `content/javascript/argos-amd-dependencies.js`, `content/javascript/argos-sdk.js`, the Dojo core under `content/dojo/`, `content/javascript/cultures/`, and the CSS themes. The Stager overlays SDK_Deploy on top of SLX_Deploy so the union forms the complete Argos_Web_App.
...
- **Stager**: The Grunt task `cordova:stage` that synchronises the contents of SLX_Deploy into Cordova_WWW, overlays SDK_Deploy on top, and rewrites the WebView entry document.
...
8. WHEN Stager runs, THE Stager SHALL overlay every file from `argos-sdk/deploy/` (SDK_Deploy) into Cordova_WWW preserving the relative directory structure, applying the same exclusion list as criteria 5 and 6, after copying SLX_Deploy and before copying Cordova_Entry_Template, so that the SDK / Dojo layer (`argos-sdk.js`, `argos-dependencies.js`, `argos-amd-dependencies.js`, `content/dojo/`, `content/javascript/cultures/`, CSS themes) the entry template references is present in Cordova_WWW.
9. WHERE a relative path exists in both SLX_Deploy and SDK_Deploy, THE Stager SHALL let the SDK_Deploy file win, matching the production IIS overlay order (unstash `slx`, then `sdk`).
10. IF `argos-sdk/deploy/` does not exist when Stager runs, THEN THE Stager SHALL fail with a non-zero exit code and an error message instructing the operator to run the argos-sdk release build first.
11. THE Stager SHALL not modify any file under `argos-sdk/deploy/`.
...
4. THE Cordova_Config SHALL declare a Content Security Policy meta tag value in the staged `index.html` that allows `'self'`, `gap:`, and the configured Server_Endpoint hosts, that allows `'unsafe-eval'` for the Dojo AMD loader, and that allows `'unsafe-inline'` in `script-src` because the entry document (mirroring the web `index.html`) executes inline `<script>` blocks (the Dojo AMD `require({...})` configuration, the Soho config, the pdf.js worker assignment, and the `deviceready` bootstrap guard). Without `'unsafe-inline'` the WebView blocks those inline scripts and the application never bootstraps.
